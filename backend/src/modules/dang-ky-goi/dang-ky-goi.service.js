...
function kiemTraBatDauLuc(value) {
    ...
    if (Number.isNaN(date.getTime())) { throw taoLoi(400, 'Thời gian bắt đầu không hợp lệ.', MA_LOI.THOI_GIAN_KHONG_HOP_LE); }
    if (date.getTime() > Date.now()) { throw taoLoi(400, 'Thời gian bắt đầu không được lớn hơn thời gian hiện tại.', MA_LOI.THOI_GIAN_BAT_DAU_TUONG_LAI); }
    return date;
}
...
async function layGoiHopLe(goiDichVuId, db = null) {
    ...
    if (!goi.active) { throw taoLoi(400, 'Gói dịch vụ hiện không còn được phép đăng ký.', MA_LOI.GOI_DICH_VU_KHONG_HOAT_DONG); }
    return goi;
}
...
async function getChiTiet(id) {
    ...
    if (!dangKy) { throw taoLoi(404, 'Đăng ký gói không tồn tại.', MA_LOI.DANG_KY_GOI_KHONG_TIM_THAY); }
    return dangKy;
}
...
async function dangKy(nguoiDungId, data) {
    ...
    return giaoDich(async (db) => {
        ...
        if (dangMo) { throw taoLoi(409, 'Bạn đã có đăng ký chưa kết thúc đối với gói dịch vụ này.', MA_LOI.DANG_KY_GOI_DA_TON_TAI); }
        ...
        if (goiHienTai) { throw taoLoi(409, 'Bạn đang có một gói dịch vụ hoạt động. Hãy kết thúc hoặc thay đổi gói hiện tại trước.', MA_LOI.DA_CO_GOI_HOAT_DONG); }
...
async function ganGoi(data) {
    ...
    return giaoDich(async (db) => {
        ...
        const goi = await layGoiHopLe(goiId, db);
        const dangMo = await repository.getDangMoCuaNguoiDungVaGoi(userId, goiId, db);
        if (dangMo) { throw taoLoi(409, 'Người dùng đã có đăng ký chưa kết thúc đối với gói này.', MA_LOI.DANG_KY_GOI_DA_TON_TAI); }
        const goiHienTai = await repository.getHienTaiCuaNguoiDung(userId, db);
        if (goiHienTai && !data.thayTheGoiHienTai) { throw taoLoi(409, 'Người dùng đang có một gói dịch vụ hoạt động.', MA_LOI.DA_CO_GOI_HOAT_DONG); }
        const dangKyMoi = await repository.create({
            nguoiDungId: userId,
            goiDichVuId: goiId,
            trangThai: 'CHO_THANH_TOAN',
...
async function kichHoat(id, data = {}) {
    ...
    return giaoDich(async (db) => {
        ...
        if (!dangKy) { throw taoLoi(404, 'Đăng ký gói không tồn tại.', MA_LOI.DANG_KY_GOI_KHONG_TIM_THAY); }
        if (dangKy.trangThai !== 'CHO_THANH_TOAN') { throw taoLoi(400, 'Chỉ đăng ký đang chờ thanh toán mới có thể kích hoạt.', MA_LOI.TRANG_THAI_DANG_KY_KHONG_HOP_LE); }
        ...
        if (goiHienTai && goiHienTai.id !== dangKyId && !data.thayTheGoiHienTai) { throw taoLoi(409, 'Người dùng đang có một gói dịch vụ hoạt động.', MA_LOI.DA_CO_GOI_HOAT_DONG); }
        ...
        if (!ketQua) { throw taoLoi(400, 'Không thể kích hoạt đăng ký gói.', MA_LOI.KHONG_THE_KICH_HOAT_GOI); }
        return ketQua;
    });
}
...
async function tamDung(id) {
    ...
    const dangKy = await repository.getById(dangKyId);
    if (!dangKy) { throw taoLoi(404, 'Đăng ký gói không tồn tại.', MA_LOI.DANG_KY_GOI_KHONG_TIM_THAY); }
    if (dangKy.trangThai !== 'HOAT_DONG') { throw taoLoi(400, 'Chỉ gói đang hoạt động mới có thể tạm dừng.', MA_LOI.TRANG_THAI_DANG_KY_KHONG_HOP_LE); }
    const ketQua = await repository.tamDung(dangKyId);
    if (!ketQua) { throw taoLoi(400, 'Không thể tạm dừng gói dịch vụ.', MA_LOI.KHONG_THE_TAM_DUNG_GOI); }
    return ketQua;
}
...
async function tiepTuc(id) {
    ...
    return giaoDich(async (db) => {
        ...
        if (!capNhat || capNhat.trangThai !== 'TAM_DUNG') { throw taoLoi(400, 'Gói không ở trạng thái tạm dừng hoặc đã hết hạn.', MA_LOI.TRANG_THAI_DANG_KY_KHONG_HOP_LE); }
        ...
        if (goiHienTai && goiHienTai.id !== dangKyId) { throw taoLoi(409, 'Người dùng đang có một gói dịch vụ khác hoạt động.', MA_LOI.DA_CO_GOI_HOAT_DONG); }
        const ketQua = await repository.tiepTuc(dangKyId, db);
        if (!ketQua) { throw taoLoi(400, 'Không thể tiếp tục gói dịch vụ.', MA_LOI.KHONG_THE_TIEP_TUC_GOI); }
        return ketQua;
    });
}
...
async function huyCuaToi(id, nguoiDungId, data = {}) {
    ...
    const userId = parseId(nguoiDungId, 'ID người dùng');
    const dangKy = await repository.getById(dangKyId);
    if (!dangKy || dangKy.nguoiDungId !== userId) { throw taoLoi(404, 'Đăng ký gói không tồn tại.', MA_LOI.DANG_KY_GOI_KHONG_TIM_THAY); }
    const ketQua = await repository.huy(dangKyId, data.lyDo || 'Người dùng hủy gói', userId);
    if (!ketQua) { throw taoLoi(400, 'Gói dịch vụ không thể hủy ở trạng thái hiện tại.', MA_LOI.KHONG_THE_HUY_GOI); }
    return ketQua;
}
