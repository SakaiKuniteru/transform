...
const renderer = require('./powerpoint.renderer');

const DINH_DANG_POWERPOINT = parser.DINH_DANG_POWERPOINT;
const DINH_DANG_DICH = Object.freeze([DINH_DANG.PDF, DINH_DANG.PNG, DINH_DANG.JPG, DINH_DANG.JPEG]);
let libreOfficePromise = null;
let popplerPromise = null;
...
function layExtension(dinhDang) { return dinhDang === DINH_DANG.JPEG ? 'jpg' : dinhDang; }
...
async function taoDauRa(context, ketQua) {
    ...
    const storage = await storageService.luuTuBuffer(khoa, ketQua.buffer, { contentType: ketQua.mimeType, metadata: { congViecId: context.congViecId ? String(context.congViecId) : '', buocId: context.buocId ? String(context.buocId) : '', dinhDang: ketQua.dinhDang, boXuLy: 'powerpoint' } });
...
async function coPoppler() { if (!popplerPromise) { popplerPromise = poppler.kiemTra().then(() => true).catch(() => false); } return popplerPromise; }

async function hoTroPowerPoint(context = {}) {
    const nguon = String(context.dinhDangNguon || '').trim().toLowerCase();
    const dich = String(context.dinhDangDich || '').trim().toLowerCase();
    if (!DINH_DANG_POWERPOINT.includes(nguon) || !DINH_DANG_DICH.includes(dich)) { return false; }
    if (!await coLibreOffice()) { return false; }
    if (dich !== DINH_DANG.PDF && !await coPoppler()) { return false; }
    return true;
}
...
async function xuLyChuyenDinhDang(context) {
    ...
    const dich = String(context.dinhDangDich || '').trim().toLowerCase();
    if (!DINH_DANG_DICH.includes(dich)) { throw taoLoi(415, `Định dạng PowerPoint đích "${dich || 'không xác định'}" chưa được hỗ trợ.`, MA_LOI.TEP_DINH_DANG_KHONG_HO_TRO); }
    await context.capNhatTienTrinh(20);
    const ketQua = dich === DINH_DANG.PDF ? await renderer.chuyenSangPdf(buffer, metadataNguon.dinhDang, { ...context.tuyChon, signal: context.signal }) : await renderer.renderSlide(buffer, metadataNguon.dinhDang, dich, { ...context.tuyChon, signal: context.signal });
    await context.kiemTraHuy();
    await context.capNhatTienTrinh(90);
    const dauRa = await taoDauRa(context, ketQua);
    return { boXuLy: 'TAI_LIEU', congCu: ketQua.thongKe?.congCu || (dich === DINH_DANG.PDF ? 'libreoffice' : 'libreoffice+pdftoppm'), phienBanCongCu: null, dauRa, dinhDangDich: dich, thongKe: { dinhDangNguon: metadataNguon.dinhDang, kichThuocNguonBytes: buffer.length, kichThuocDichBytes: ketQua.kichThuocBytes, ...(ketQua.thongKe || {}) } };
}
...
function dangKyTatCa() {
    return Object.freeze([
        dangKyNeuChuaCo({
            key: 'powerpoint:chuyen-dinh-dang',
            ten: 'Chuyển đổi PowerPoint/OpenDocument Presentation',
            loaiChuyenDoi: LOAI_CHUYEN_DOI.CHUYEN_DINH_DANG,
            nhomXuLy: 'TAI_LIEU',
            dinhDangNguon: DINH_DANG_POWERPOINT,
            dinhDangDich: DINH_DANG_DICH,
            uuTien: 100,
            chiPhi: 2,
            engine: 'powerpoint',
            phienBanEngine: null,
            hoTro: hoTroPowerPoint,
