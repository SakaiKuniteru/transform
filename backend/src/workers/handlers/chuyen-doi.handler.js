...
function taoHandler({ tenQueue, loaiXuLy, trangThaiCongViec }) {
    ...
    return async function xuLy(job) {
        ...
        const context = Object.freeze({
            ...
            buoc,
            cacBuoc,
            dauVao: buoc?.dauVao || congViec.dauVao || {},
            tuyChon: { ...(congViec.tuyChon || {}), ...(buoc?.tuyChon || {}) },
            jobData: chuanHoaObject(job.data),
            capNhatTienTrinh,
            kiemTraHuy: () => kiemTraHuy(congViec.id, buoc)
        });
        try {
            const engine = layEngine();
            const ketQua = chuanHoaKetQua(await engine.xuLy(context));
            await kiemTraHuy(congViec.id, buoc);
            await capNhatTienTrinh(100);
            if (buoc) {
                await congViecService.capNhatBuoc(buoc.id, {
                    ...
                    thongKe: chuanHoaObject(ketQua.thongKe),
                    soLanThu: thongTinLanThu.lanThu,
                    danhDauHoanThanh: true
                });
            }
            if (!buoc || ketQua.hoanTatCongViec === true) {
                await congViecService.hoanThanh(congViec.id, {
            ...
            if (error?.maLoi === MA_LOI.CONG_VIEC_DA_HUY) { return error.chiTiet || { daHuy: true, congViecId: congViec.id, buocId: buoc?.id || null }; }
            ...
            if (buoc) {
                await congViecService.capNhatBuoc(buoc.id, {
                    ...
                    thongBaoLoi,
                    chiTietLoi,
                    danhDauHoanThanh: thongTinLanThu.laLanCuoi
                });
            }
            if (thongTinLanThu.laLanCuoi) {
                await congViecService.thatBai(congViec.id, { maLoi, thongBaoLoi, chiTietLoi });
            } else {
